create or refresh streaming table gold.reviews_and_tips_chunked as (
    select
        reviews_and_tips.text_id,
        reviews_and_tips.business_id,
        chunks.pos as chunk_id,
        md5(reviews_and_tips.text_id || '|' || chunk_id) as pk,
        substring(reviews_and_tips.full_text, 1 + chunks.pos * 130, 150) as text_chunk
    from stream(genieology.gold.reviews_and_tips)
